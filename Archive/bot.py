...
MAIN_GUILD_ID = None # Guild ID for Ophelia's main server
category_id = 1131177517974290553 # ID for User Channel category in main server
...
async def sync(interaction: discord.Interaction):
    if interaction.user.id == None or None: #DOOT DOOT MISSING DOOT DOOT
        await bot.tree.sync()
...
async def on_application_command_error(ctx, error):
    ...
    embed = discord.Embed(title= "**An error has occured. Please try again!**", 
                        description= "If this problem presists please report the issue using the </feedback:1131265027052142670> command",
...
@app_commands.choices(u_selection=[
    app_commands.Choice(name="Rin", value= "A"),
    app_commands.Choice(name="Rem", value= "B"),
    app_commands.Choice(name="Rei", value= "C")
])
async def personality(interaction: discord.Interaction,  u_selection: app_commands.Choice[str]):
    if modelMV3.usernameExists(str(interaction.user.id)) == True:
        db.reference(f"/{interaction.user.id}").update({"Preset": u_selection.value})
...
class SimpleView(discord.ui.View):
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        db.reference(f"/{interaction.user.id}/Memories").set([{"role":"system", "content": modelMV3.personaA}]) #Typo 1
...
async def clear_memories(ctx):
    if modelMV3.usernameExists(str(ctx.author.id)) == True:
    ...
    if modelMV3.usernameExists(str(interaction.user.id)) == True:
        if interaction.guild == None:
            ...
            if u_selection.value == "False":
                ...
                db.reference(f"/{interaction.user.id}").update({"Visibility": True})
...
@app_commands.choices(u_selection=[
    ...
    app_commands.Choice(name="Only to the /say command", value= "False")
])
...
async def on_message(message):
    if message.guild == bot.get_guild(MAIN_GUILD_ID):
        ...
        if message.channel.category_id == category_id:
            ...
            ref = db.reference(f"/{message.author.id}/AutoReply")
            msg_mode = ref.get()

            if msg_mode == True:

                response = modelMV3.callModel(str(message.author.id), message.content)
...
async def talk(interaction: discord.Interaction, text_to_send : str):
    if modelMV3.usernameExists(str(interaction.user.id)) == True:
            if interaction.guild == bot.get_guild(MAIN_GUILD_ID) or None:
                ...
                response = modelMV3.callModel(str(interaction.user.id), text_to_send)
                if response == False:
                ...
                ref = db.reference(f"/{interaction.user.id}/Visibility")
                status = ref.get()
...
async def register(interaction: discord.Interaction):
    ...
    if modelMV3.registerAcc(str(interaction.user.id)) == True:
...
async def account_info(interaction: discord.Interaction):
    ...
    if modelMV3.usernameExists(str(interaction.user.id)) == True:
        ...
        balance = db.reference(f"/{str(interaction.user.id)}/Balance").get()
        userTokens = db.reference(f"/{str(interaction.user.id)}/TotalUserTokens").get()
        accType = db.reference(f"/{str(interaction.user.id)}/Type").get()
...
class FeedbackModal(discord.ui.Modal, title="Feedback & Report Terminal"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        MAIN_GUILD = bot.get_guild(1131172068927479850)
